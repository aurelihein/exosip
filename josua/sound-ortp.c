...
void *os_sound_start_thread(void *_ca)
{
	...
	while (ca->enable_audio != -1) {
		...
		/* receive ONE packet */
		i = rtp_session_recv_with_ts(ca->rtp_session, data_in, 160, timestamp,
									 &have_more);
...
void *os_sound_start_thread(void *_ca)
{
	...
	while (ca->enable_audio != -1) {
		...
		do {
			memset(data_in, 0, 160);
			i = rtp_session_recv_with_ts(ca->rtp_session, data_in, 160,
										 timestamp, &have_more);
			if (i > 0) {
...
void *os_sound_start_out_thread(void *_ca)
{
	...
	while (ca->enable_audio != -1) {
		...
		for (k = 0; k < ca->speex_nb_packet; k++) {
			...
			if (i > 0) {
				for (j = 0; j < ca->speex_fsize; j++) {	/* convert to float */
