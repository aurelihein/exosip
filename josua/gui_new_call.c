...
void
window_new_call_with_to (char *_to)
{
...
window_new_call_print ()
{
  ...
  mvaddnstr (gui_window_new_call.y0 + 1,
             ...
             static_to, x - gui_window_new_call.x0 - 1 - 10);


  attrset (COLOR_PAIR (1));
  snprintf (buf, x - gui_window_new_call.x0, "Subject : ");
  mvaddnstr (gui_window_new_call.y0 + 2,
             gui_window_new_call.x0, buf, x - gui_window_new_call.x0 - 1);
  snprintf (buf, x - gui_window_new_call.x0, "Route   : ");
  mvaddnstr (gui_window_new_call.y0 + 3,
             gui_window_new_call.x0, buf, x - gui_window_new_call.x0 - 1);
  snprintf (buf, x - gui_window_new_call.x0, "Attchmnt: ");
  mvaddnstr (gui_window_new_call.y0 + 4,
             gui_window_new_call.x0, buf, x - gui_window_new_call.x0 - 1);
...
int
window_new_call_run_command (int c)
{
  int y, x;
  int i;
      ...
      case 4:                  /* Ctrl-D */
        {
          ...
          mvaddnstr (gui_window_new_call.y0 + gui_window_new_call.ycursor,
                     gui_window_new_call.x0 + 10,
                     buf, x - gui_window_new_call.x0 - 10 - 1);
          gui_window_new_call.xcursor = 10;
        }
        break;
      case 5:                  /* Ctrl-E */
      ...
      case 16:                 /* Ctrl-P */
        {
          ...
          mvinnstr (ycur, xcur, to, x - gui_window_new_call.x0 - 10);
          ycur++;
          mvinnstr (ycur, xcur, refer_to, x - gui_window_new_call.x0 - 10);
          ycur++;
          mvinnstr (ycur, xcur, route, x - gui_window_new_call.x0 - 10);

          i = _josua_start_refer (refer_to, cfg.identity, to, route);
          if (i != 0)
            beep ();
          /* mvinnstr(ycur, xcur, tmp, 199); */
        }
