...
static void josua_clear_commands_only()
{
	int x, y;
	char *commands[] = { NULL };
	getmaxyx(stdscr, y, x);
...
static void __show_toggle_online_logline()
{
...
static int josua_gui_key_pressed()
{
	int c;
	int i;
...
int gui_start()
{
	...
	for (;;) {
		...
		for (i = 0; gui_windows[i] != NULL && gui_windows[i]->on_off != GUI_ON;
			 i++) {
		}
