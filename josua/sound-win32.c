...
/* !		fprintf(stderr, "__call_free: waveOutOpen: 0x%i\n", mr); */
		...
		for (i = 0; i < USED_OUT_BUFFERS; i++) {
			...
			mr = waveOutPrepareHeader(hWaveOut, &(waveHdrOut[i]),
									  sizeof(waveHdrOut[i]));
			if (mr != MMSYSERR_NOERROR) {
/* !				fprintf(stderr, "__call_free: waveOutPrepareHeader: 0x%i\n", mr); */
	...
	if (gsmformat != NULL)
		mr = waveInOpen(&hWaveIn, waveinDeviceID,
						(LPWAVEFORMATEX) & (gsmformat->wfx),
...
/* !		fprintf(stderr, "__call_free: waveInOpen: 0x%i\n", mr); */
		...
		for (i = 0; i < MAX_IN_BUFFERS; i++) {
			...
			mr = waveInPrepareHeader(hWaveIn, &(waveHdrIn[i]),
									 sizeof(waveHdrIn[i]));
			if (mr == MMSYSERR_NOERROR) {
				mr = waveInAddBuffer(hWaveIn, &(waveHdrIn[i]),
									 sizeof(waveHdrIn[i]));
				if (mr == MMSYSERR_NOERROR) {
/* !					fprintf(stderr, "__call_free: waveInAddBuffer: 0x%i\n", mr); */
					/* return -1; */
...
void *os_sound_start_thread(void *_ca)
{
	...
	while (ca->enable_audio != -1) {
		...
		i = rtp_session_recv_with_ts(ca->rtp_session, data_in, length, timestamp,
									 &have_more);
		if (i > 0) {
			MMRESULT mr = NOERROR;
