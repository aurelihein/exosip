...
int open_sndcard(int format)
{
	...
	*/
	...
	if (gsmformat!=NULL)
		mr = waveOutOpen(&hWaveOut, waveoutDeviceID, (LPWAVEFORMATEX)&(gsmformat->wfx), (DWORD)0/* SpeakerCallback */, 0/* arg */, CALLBACK_NULL /* CALLBACK_FUNCTION */);
	else
	...
	if (gsmformat!=NULL)
		mr = waveInOpen(&hWaveIn, waveinDeviceID, (LPWAVEFORMATEX)&(gsmformat->wfx), (DWORD)WaveInCallback, 0, CALLBACK_FUNCTION);
	else
  ...
  while (ca->enable_audio != -1)
    {
      ...
      i = rtp_session_recv_with_ts(ca->rtp_session, data_in, length, timestamp, &have_more);
      if (i>0) {
	  	MMRESULT mr = NOERROR;
...
int os_sound_start(jcall_t *ca)
{
  ...
  else return -1; /* not supported codec?? */
