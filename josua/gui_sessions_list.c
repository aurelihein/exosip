...
int window_sessions_list_print()
{
	...
	for (k = 0; k < MAX_NUMBER_OF_CALLS; k++) {
		...
		if (pos >
			y + gui_window_sessions_list.y1 - gui_window_sessions_list.y0 + 1)
...
int window_sessions_list_run_command(int c)
{
	...
	case 'R':
		...
		eXosip_lock();
		{
			...
			i = eXosip_call_build_refer(ca->did, "sip:800@192.168.2.2:5070",
										&refer);
	...
	case 'E':
		...
		eXosip_lock();
		{
			...
			i = eXosip_call_build_notify(ca->did, EXOSIP_SUBCRSTATE_TERMINATED,
										 &notify);
			if (i == 0) {
	...
	case 'T':
		...
		eXosip_lock();
		{
			...
			i = eXosip_call_build_refer(ca->did, "sips:800@192.168.2.2:5000",
										&refer);
			if (i == 0)
				i = eXosip_call_send_request(ca->did, refer);
			if (i != 0)
