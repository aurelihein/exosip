...
void sdes_print(struct rtp *session, uint32_t ssrc, rtcp_sdes_type stype)
{
...
void packet_print(struct rtp *session, rtp_packet * p)
{
#ifdef USE_PCM
...
void rtp_event_handler(struct rtp *session, rtp_event * e)
{
	rtp_packet *p;
	rtcp_sdes_item *r;
...
void *os_sound_start_thread(void *_ca, int port)
{
	int i;
